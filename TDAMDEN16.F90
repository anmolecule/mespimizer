!  Copyright 2013-2016, Jaime Fernandez Rico, Rafael Lopez, Ignacio Ema,
!  Guillermo Ramirez
! 
!  This file is part of DAM2016.
! 
!  DAM2016 is free software: you can redistribute it and/or modify
!  it under the terms of the GNU General Public License as published by
!  the Free Software Foundation, either version 3 of the License, or
!  (at your option) any later version.
! 
!  DAM2016 is distributed in the hope that it will be useful,
!  but WITHOUT ANY WARRANTY; without even the implied warranty of
!  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
!  GNU General Public License for more details.
! 
!  You should have received a copy of the GNU General Public License
!  along with DAM2016.  If not, see <http://www.gnu.org/licenses/>.
!
!------------------------------------------------------------------------
!
!> @author Rafa
...
!	real(KREAL4) :: tarray(2), tiempo, dtime
	namelist / options /  iatomsel, latomics &
		, latomsel, lboundsx, lboundsy, lboundsz, ldensacc, lmaxrep, lminrep &
		, lmolec, longoutput, nsel, numrtab, rtab, umbrlargo 
...
!	lexact  = .false.		! if .true. "exact" density is tabulated
	...
	lmaxrep = lmaxi			! highest "l" in the expansion of the density. It does not hold if "lexact .eq. true"
...
!		lnamelist = (/ langstrom, lgrid, lgradient, laplacian, lderiv2, lexact, ldensacc, latomics, latomsel, lmolec /)
...
!	endif
	...
	if (ierr .ne. 0) then
		call error(1,"Memory error when allocating lselat in gridpot") 
	endif
...
!	selected atomic fragments for tabulation	
		...
		elseif (latomics ) then
			...
			if (ncen .gt. mxsel) then
					write(iout,"('WARNING: Number of selected atoms larger than allowed for tabulation of the atomic densities')")
					write(iout,"('Only the first ',i3, ' atomic densities will be tabulated')") mxsel
				nsel = mxsel
			endif
			do i = 1, nsel
				lselat(i) = .true.
		...
		if (latomics) write(iout,"('Indices of atoms selected for tabulation = ', 20(i4))") (iatomsel(i), i = 1, nsel)
		if (ldensacc) write(iout,"('Indices of atoms selected for density accumulation = ', 20(i4))") (iatomsel(i), i = 1, nsel) 
	if (lminrep .gt. lmaxexp .and. .not. lexact) then
			write(iout,"('lminrep = ', i3, ' greater than lmaxexp ', i3)") lminrep, lmaxexp
			write(iout,"('takes lminrep = 0')")
		lminrep = 0
	endif
	if (.not. lexact) write(iout,"('Expansion of the density: lminrep = ', i3, ' : lmaxrep = ', i3)") &
		lminrep, lmaxrep	
	if (lexact) then
...
! 		Allocates auxiliary arrays for exact density
		...
		if (ierr .ne. 0) then
			call error (1,"Memory error when allocating f and faux")
		endif
...
!		Allocates the array containing the density matrix
		...
		if (ierr .ne. 0) then
			call error(1,"Memory error when allocating dmat")
		endif
		if (longoutput) write(iout,"('Estimated highest size of dmat   = ', i15, ' bytes')") size(dmat)
		...
		if (lSTO) then
			...
			if (ierr .ne. 0) then
				call error(1,"Cannot open file den file") 
			else
		...
		if ( ios .ne. 0 .or. nbas .ne. nbasis ) then
			call error(1,"ERROR reading density matrix. &
...
!	Tabulates specific points if required
		...
		if (ierr .eq. 0) then
			if (lexact) then
!				write(iout,"(//12x,'Density from the basis set and density matrix' &
!					,/12x, 45('='), //9x,'X',17x,'Y',17x,'Z',20x,'den')")
			else
				if (lgradient) then
					if (laplacian) then
!						write(iout,"(//12x,'Density expansion from l = ',i2, ' to  l = ',i2,/10x, 45('='),  &
...
!				open(9,file=trim(projectname)//"-d.der2",form='formatted', iostat=ierr)
				if (ierr .ne. 0) then
!					write(iout,"('Cannot open file ', a, ' in processor ',i3)") trim(projectname)//"-d.der2", myrank
					lderiv2 = .false.
				endif
			endif
			if (numrtab .gt. mxrtab) then
!				write(iout,"('Number of tabulation points numrtab = ', i4, ' exceeds the highest allowable = ', i4 &
...
!							write(9,"('laplacian comprobation ',2(1x,e22.15))") denlp, dxxden + dyyden + dzzden
					...
					if (laplacian) then
!						write(iout,"(3(1x,e17.10), 3x, d22.15, 4(1x,e22.15),/)") x, y, z, den, dxden, dyden, dzden, denlp
					else
!						write(iout,"(3(1x,e17.10), 3x, d22.15, 3(1x,e22.15),/)") x, y, z, den, dxden, dyden, dzden
					endif
				else
					if (laplacian) then
!						write(iout,"(3(1x,e17.10), 3x, d22.15,1x, e22.15/)") x, y, z, den, denlp
					else
!						write(iout,"(3(1x,e17.10), 3x, d22.15,/)") x, y, z, den
					endif
				endif
				if (ipoint .le. numrtab) then
					x = rtab(1,ipoint)
...
!
  subroutine leedamqtden
	...
	integer(KINT) :: l, lenindintrv, lm, m, ncfaj, ncflm, nsamples, nsize
	...
	if (ierr .ne. 0) then
		call error (1,"Error when inquiring damqt file")
		return
	endif
	if (nsize .eq. -1) then
...
!	allocate(atmnam(ncen), atwt(ncen), nzn(ncen), rcen(3,ncen), zn(ncen), stat = ierr) !AK27June2015
	if (ierr .ne. 0) then
		call error(1,"Memory error when allocating atmnam, nzn, rcen and zn") 
		return
	endif

	write(iout,"(27x,'GEOMETRY')")
...
!	Geometry and nuclear charges
	do ia = 1, ncen
		...
		write(iout,"(t6, i3, t13, f12.7, t25, f12.7, t37, f12.7, t51, f10.5)") &
...
!			xx(ncaps), stat = ierr)
		if (ierr .ne. 0) then
			call error(1,"Memory error when allocating ll, lmaxc, nf, ngini, ngfin, nn, rlargo, rnor and xx")
			return
		endif
...
!			ngfin(ncen), nprimit(ncaps), rlargo(ncen), rnor(ncaps), xxg(nprimitot), stat = ierr)
		if (ierr .ne. 0) then
			call error(1,"Memory error when allocating cfcontr, ipntprim, ll, lmaxc, ncontr, nf, ngini, ngfin, &
...
!				computes and stores the radial normalization factor
	...
	if (lexact) then
			do ia = 1, ncen
				write(iout,"('long range radius of basis set for atom ', i4, ': ', e12.5)") ia, rlargo(ia)
			enddo
		return
	endif
...
!	Data of density representation
	...
	nsize = nsize - sizeof(icfposd) - sizeof(xajustd)
	
	allocate(cfajust(nsize/8), stat = ierr)
	if (ierr .ne. 0) then
		call error(1,"Memory error when allocating cfajust") 
		return
	endif
	
	do ia = 1, ncen      ! Do over centers
		read(10) icfposd(1:lmtop*nintervaj+1,ia)
		if (ia .gt. 1) icfposd(1:lmtop*nintervaj+1,ia) = icfposd(1:lmtop*nintervaj+1,ia) + icfposd(lmtop*nintervaj+1,ia-1) - 1
...
!	Generates an auxiliary index array for determining the interval to which a given r belongs
	step = rinterv(1)
	fct = uno / step
	lenindintrv = int(rinterv(nintervaj) * fct)
	
	allocate(indintrv(lenindintrv), stat = ierr)
	if (ierr .ne. 0) then
		call error(1,"Memory error when allocating indintrv")
		return
	endif
...
!	Determines the long-range radii and the highest l in the expansion for each interval
	...
	if (ierr .ne. 0) then
		call error(1,"Memory error when allocating lcorto") 
		return
	endif
...
!   Density tabulation in point (x,y,z) from density matrix and basis set	
   subroutine densexactaSTO(x, y, z, denex)
	...
	USE DAM2016_DATA_T
	implicit none
	integer(KINT) :: i, i1, ia, ib, ierr, j, knt, kntb, l, m 
	real(KREAL) :: denex, frad, ra, ra2, x, xxa, y, yya, z, zza
	real(KREAL) :: rapow(0:mxn), zlma((mxlexp+2)**2)
	logical :: ldenex
...
!   Density tabulation in point (x,y,z) from density matrix and basis set	
   subroutine densexactaGTO(x, y, z, denex)
	...
	USE GAUSS_T
	implicit none
	integer(KINT) :: i, i1, i1p, ia, ib, ierr, j, knt, kntb, l, m 
	real(KREAL) :: denex, frad, ra, ra2, x, xxa, y, yya, z, zza
	real(KREAL) :: zlma((mxlexp+2)**2)
	logical :: ldenex
...
!	Contribution of atomic fragment ia to density  in point (x,y,z)
	...
	if (lgradient .or. laplacian) then
		...
		t = dos * (ra - rinterv(interv-1))/(rinterv(interv)-rinterv(interv-1)) - uno
		dost = t + t
! write(6,"('interv = ', i3, ' t = ', e17.10, ' uno = ', e17.10, ' ra = ', e17.10, ' rinterv(interv-1) = ', e17.10)") &
! interv, t, uno, ra, rinterv(interv-1)
! call flush(6)
! 		umt2i = uno / (t*t - uno)
