...
! #define DBLPRCGRID	! Uncomment this line  if double precision grid is wanted
  subroutine PREDAMPOT
	...
	real(KREAL) :: aux 
	namelist / options / geomthr, lmaxrep, longoutput, numrtab, rtab, umbrlargo 
	...
	if (.not. lexact) write(iout,"('Potential from expansion of the density: lmaxrep = ', i3)") lmaxrep
	...
	if (ierr .ne. 0) then
		call error(1,"Memory error when allocating ra2l1 and ra2l1inv")
	endif
	if (lexact) then
		allocate(rl(-2*mxl:2*mxl,-2*mxl:2*mxl,0:2*mxl), dl(-2*mxl:2*mxl,-2*mxl:2*mxl,0:2*mxl), stat = ierr)
		if (ierr .ne. 0) then
			call error(1,"Memory error when allocating rl and dl") 
		endif
	endif
    return
    END	
...
! #define DBLPRCGRID	! Uncomment this line  if double precision grid is wanted
  subroutine DAMPOT(vtot,drvxtot,drvytot,drvztot,dxxtot,dxytot,dxztot,dyytot,dyztot,dzztot,x,y,z)
	USE DAM2016_T
	USE DAM2016_CONST_T
	USE DAM2016_DATA_T
	USE DAMPOT2016_T
...
!	Tabulates specific points if required
		if (largo .and. .not. lexact) then       ! Long-range potential
			if (lgradient) then
!				write(iout,"(//12x,'Long-range electrostatic potential from density expansion up to  l = ',i2,/10x, 95('='),  &
!				//9x,'X',17x,'Y',17x,'Z',t65,'Vnuc',t88,'Vel',t111,'Vtot',t133,'der x', t156, 'der y', t180, 'der z')") &
!				lmaxrep 
			else
!				write(iout,"(//12x,'Long-range electrostatic potential from density expansion up to  l = ',i2 &
!				,/10x, 71('='), //9x,'X',17x,'Y',17x,'Z',19x,'Vnuc',19x,'Vel',19x,'Vtot')") lmaxrep
			endif
		else
			if (.not. lexact) then
				if (lgradient) then
!					write(iout,"(//12x,'Electrostatic potential from density expansion up to  l = ',i2,/10x, 95('='),  &
!					//9x,'X',17x,'Y',17x,'Z',t65,'Vnuc',t88,'Vel',t111,'Vtot',t133,'der x', t156, 'der y', t180, 'der z')") &
!					lmaxrep 
				else
!					write(iout,"(//12x,'Electrostatic potential from density expansion up to  l = ',i2,  &
!					/10x, 71('='), //9x,'X',17x,'Y',17x,'Z',19x,'Vnuc',19x,'Vel',19x,'Vtot')") lmaxrep
				endif
			else
!				write(iout,"(//12x,'Electrostatic potential from density and basis set'  &
...
!			if (ierr .ne. 0) then
!				write(iout,"('Cannot open file ', a, ' in processor ',i3)") trim(projectname)//"-v.der2", myrank
!				lderiv2 = .false.
!			endif
!		endif
		...
		if (ipoint .le. numrtab) then
			...
			if (lexact) then
				call potexactGTO(x, y, z, vntot, vetot, vtot)
!				write(iout,"(3(1x,e17.10), 3(1x,e22.15))") x, y, z, vntot, vetot, vtot
			else
				vntot = cero
				vetot = cero
				vtot = cero
				...
				do ia = 1, ncen
					if (largo) then
						call potlargo(ia, x, y, z, vn, ve, vtotia, drvx, drvy, drvz, dxx, dxy, dxz, dyy, dyz, dzz)
					else
						call potencial(ia, x, y, z, vn, ve, vtotia, drvx, drvy, drvz, dxx, dxy, dxz, dyy, dyz, dzz)
					endif
					if (lgradient) then
...
!					write(iout,"(3(1x,e17.10), 3(1x,e22.15), 3(1x,e22.15))") x, y, z, vntot, vetot, vtot, &
...
!     into functions of the same type
	...
	do m2 = -mxemes, mxemes
		do m1 = -mxemes, mxemes
			call emes ( m1, m2, ms, md, ss, sd, extiden )
			msv(m1,m2) = ms
			mdv(m1,m2) = md
			ssv(m1,m2) = ss
			sdv(m1,m2) = sd 
...
!
  subroutine leedamqtpot
	...
	integer(KINT) :: l, lenindintrv, lm, m, nbasis, ncflm, nsamples, nsize
	...
	if (ierr .ne. 0) then
		write(iout,"('Error when inquiring file ', a)") trim(projectname)//"_2016.damqt"
	    call error(1,"file inquiry error")	
		return
	endif
	if (nsize .eq. -1) then
		write(iout,"('Size of file ', a, ' cannot be determined')") trim(projectname)//"_2016.damqt"
		call error(1,"file size determination error")
		return
	endif
	if ( longoutput) write(iout,"('Size of file ', a, ' = ', i12)") trim(projectname)//"_2016.damqt", nsize
#if _WIN32
	...
	if (ierr .ne. 0) then
		...
		call error(1,"Error opening damqt file")
		return
	endif
#if _WIN32
...
#endif
	if (ierr .ne. 0) then
		write(iout,"('Error when inquiring file ', a)") trim(projectname)//"_2016.dmqtv"
	    call error(1,"file inquiry error")	
		return
	endif
	if ( longoutput) write(iout,"('Opens files ', a, ' and ', a)") trim(projectname)//"_2016.damqt", &
...
!	Allocates memory for geometry
	...
	if (ierr .ne. 0) then
		write(iout,"('Memory error when allocating atmnam, nzn, rcen or zn')") 
	    call error(1,"Memory error")
		return
	endif
...
!	Basis set
	...
	if (lsto) then
		if (lexact) then
			write(iout,"('Exact potential not prepared for STO yet. Stop')")
			call error(1,"Exact potential not prepared for STO yet")
			return
		endif
...
!		Allocates memory for the basis set
		...
		if (ierr .ne. 0) then
			call error(1,"Memory error when allocating ll, lmaxc, nf, ngini,&
                        & ngfin, nn, rnor or xx")
			return
		endif
...
!	Data of density representation
	...
	if (ierr .ne. 0) then
		call error(1,"Memory error when allocating icfposd") 
		return
	endif
	...
	if (longoutput) write(iout,"('radii of fitting intervals: ',/, 8(1x,e17.10))") rinterv
...
!	Reads auxiliary integrals from file .dmqtv
	...
	if (ierr .ne. 0) then
		call error(1,"Memory error when allocating cfrint1, cfrint2l2, QGacum, Qgpart, &
		&qpacum, qppart, rlargo and rmultip") 
		return
	endif
...
!	Determines the long-range radii and the highest l in the expansion for each interval
	allocate(lcorto(nintervaj,ncen), llargo(0:mxlargo,ncen), Qlcorto(0:lmaxrep), stat = ierr )
	if (ierr .ne. 0) then
		call error (1,"Memory error when allocating lcorto, llargo, auxll,  and Qlcorto")
		return
	endif
!	long-range radii
	...
	if (longoutput) then
		...
		write(iout,"('Size of Qlcorto   = ', i15, ' bytes')") size(Qlcorto)
	endif
	nsamples = 4
	do ia = 1, ncen
		rlargo(ia) = cero
		...
		do i = 1, mxlargo
			...
			do l = 0, lmaxrep
				...
				do m = -l, l
					...
					summ1 = summ1 + abs(rmultip(kntlm,ia)) * fact(l+abs(m)) * umedpow(abs(m)) &
								* facti(l-abs(m)) * facti(abs(m))
				enddo
				Qlcorto(l) = summ1 * ral1inv
				ral1inv = ral1inv * rainv
			enddo
			llargo(i,ia) = 0
			suml1 = cero
			do l = lmaxrep, 0, -1
				suml1 = suml1 + Qlcorto(l)
			...
			enddo
		enddo
		if (longoutput) write(iout,"('llargo: ', 51(i3))") llargo(0:mxlargo,ia)
	enddo
...
!		Allocates the array containing the density matrix
		...
		if (ierr .ne. 0) then
			write(iout,"('Cannot open file ', a )") trim(projectname)//".den"
			call error(1,"Error opening den file")
		else
...
!
!   ***************************************************************
!     Calculates the electrostatic potencial from the exact density at point (x,y,z)
!
   subroutine potexactGTO(x, y, z, vnucl, vel, vtot)
	USE DAM2016_T
	USE DAMPOT2016_T
	USE DAM2016_CONST_T
	USE DAM2016_DATA_T
	USE GAUSS_T
	implicit none
	integer(KINT) :: i, i1, i1p, i2, i2p, ia, ib, indf1, k, k1, k12, k2, kmax, km1, km2 
	integer(KINT) :: l1, l12, l2, lk, lmax, ltop, m1, m1a, m2, m2a, ms, msa, md, mda
	real(KREAL) :: angnorm, aux, bux, c1, c2, cosal, cosbet, cosga, cux, expbux, funcF0, potaux, potia, potiab
	real(KREAL) :: rabinv, rra, rra2, rrab, rrab2, rrp, rrp2, rn, rn1, umur, ur, saux, sinal, sinbet, singa, suma, ss, sd
	...
	real(KREAL) :: vaux((mxl+1)**2,(mxl+1)**2), zlma((2*mxl)**2)
	...
	do ia = 1, ncen    ! Loop over the first center
		...
		call armonicos(ltop, xxa, yya, zza, zlma)	! Regular spherical harmonics		
		rra = sqrt(rra2)
		rpw(0) = uno
		do k = 1, 4*lmaxc(ia)+2
			rpw(k) = rpw(k-1) * rra
			...
			do i2 = ngini(ia), ngfin(ia)
				...
				do i1p = ipntprim(i1), ipntprim(i1)+nprimit(i1)-1
					do i2p = ipntprim(i2), ipntprim(i2)+nprimit(i2)-1
						...
						aux = rpw(2)
						call freq20(aux,bux,fv)	!  fv(i) = rra**(2i) * Gamma[i+1/2,0,(xx(i1p)+xx(i2p))*rra] / (xx(i1p)+xx(i2p))**i
						do k = 0, kmax
		...
		do ib = 1, ia-1       ! Loop over the second center
			...
			if (rrab .lt. 1.d-10) then
				write(iout,"('Error: centers ', i3, ' and ', i3, ' coincide.')") ia, ib 
				exit
			end if
...
!
!   ***************************************************************
!     Calculates the electrostatic potencial from the represented density at point (x,y,z)
!
   subroutine potencial(ia, x, y, z, vnucl, vel, vtot, drvx, drvy, drvz, dxx, dxy, dxz, dyy, dyz, dzz)
	USE DAM2016_T
	USE DAMPOT2016_T
	USE DAM2016_CONST_T
	USE DAM2016_DATA_T
	implicit none
	integer(KINT) :: i, ia, interv, icflm, j, jshft, kntlm, l, lm, lmtopot, ltop, m
...
!
!   ***************************************************************
!     Calculates the long-range electrostatic potencial on point (x,y,z)
!
   subroutine potlargo(ia, x, y, z, vnucl, vel, vtot, drvx, drvy, drvz, dxx, dxy, dxz, dyy, dyz, dzz)
	USE DAM2016_T
	USE DAMPOT2016_T
	USE DAM2016_CONST_T
	USE DAM2016_DATA_T
	implicit none
	integer(KINT) :: ia, kntlm, l, m, lm, lmtopot, ltop
