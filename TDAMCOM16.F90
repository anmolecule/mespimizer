...
!> @details Used by consta
  subroutine emes ( m1, m2, ms, md, ss, sd, extiden)
	USE DAM2016_T
	USE DAM2016_CONST_T, ONLY: uno, cero, umed
	implicit none
	integer(KINT) :: m1, m1a, m2, m2a, ms, md, extiden
	real(KREAL) :: s1, s2, s12, ss, sd
	s1 = sign(1,m1)
	s2 = sign(1,m2)
	s12 = s1 * s2
    if (extiden.EQ.1) then
	    m1a = iabs(m1)
	    m2a = iabs(m2)
	    ms = s12 * ( m1a + m2a )
	    md = s12 * iabs( m1a - m2a )
    elseif (extiden.EQ.2) then
	    m1a = abs(m1)
	    m2a = abs(m2)
	    ms = s12 * ( m1a + m2a )
	    md = s12 * abs( m1a - m2a )
